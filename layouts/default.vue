<template>
  <header class="relative w-full border-b-2 hover:shadow-md transition-shadow duration-500 bg-brand-background_white">
    <div class="flex justify-between items-center px-6 py-4">
      <img src="assets/images/Component 1.svg" alt="Site Logo" class="w-16 h-16 rounded-full shadow-lg bg-brand-primary p-2"/>
      <div class="max-sm:flex hidden cursor-pointer" @click="switch_burger">
    ...
    <nav :class="['flex justify-center gap-8 px-6 py-4 text-lg font-medium text-brand-background_white bg-brand-background_white','max-sm:flex-col max-sm:items-center max-sm:w-full',burger ? 'max-sm:flex' : 'max-sm:hidden']">
      ...
      <div class="relative group max-sm:w-full">
        ...
        <div v-show="labs" class="absolute left-0 mt-2 hidden flex-col bg-brand-primary text-brand-background_white rounded-xl shadow-lg group-hover:flex z-50 max-sm:relative max-sm:mt-1 max-sm:w-full max-sm:flex">
          <NuxtLink to="/labs" class="px-4 py-2 hover:bg-brand-accent rounded-t-xl max-sm:w-full max-sm:text-center">Lab 3</NuxtLink>
  ...
  <footer class="flex flex-row w-full bg-brand-primary border-t-2 justify-center px-4 py-2 gap-4 max-sm:flex-col max-sm:items-center max-sm:gap-2">
